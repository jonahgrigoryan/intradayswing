...
input bool   UseEngulfingFilter = true; // Use Engulfing Pattern Filter
input double MaxDailyDrawdown = 4.5;   // Maximum Daily Drawdown (%)
...
void OnTick()
{
   ...
   if(buySignal && !lastBuySignal)
   {
      ...
      Print("BUY - SL Distance: ", DoubleToString(slPips, 1), " pips | Lot Size: ", DoubleToString(lotSize, 2), " | Risk: 1% of account");
      
      // EXECUTE BUY TRADE
      if(trend == "bullish")  // Only trade in trend direction
      {
         trade.SetExpertMagicNumber(123456);  // Set magic number for EA identification
         trade.SetDeviationInPoints(10);      // Set slippage
      ...
      {
         Print("BUY signal ignored - H1 trend is not bullish");
      }
   }
   if(sellSignal && !lastSellSignal)
   {
      ...
      double lotSize = CalculateLotSize(slPips);
      Print("SELL - SL Distance: ", DoubleToString(slPips, 1), " pips | Lot Size: ", DoubleToString(lotSize, 2), " | Risk: 1% of account");
      
      // EXECUTE SELL TRADE
      if(trend == "bearish")  // Only trade in trend direction
      {
         trade.SetExpertMagicNumber(123456);  // Set magic number for EA identification
         trade.SetDeviationInPoints(10);      // Set slippage
      ...
      {
         Print("SELL signal ignored - H1 trend is not bearish");
      }
   }
   ...
   // Test MODULE 8 - Friday Trade Exit (before trailing stops)
   CloseOnFriday();
...
//+------------------------------------------------------------------+
//| MODULE 2 - M15 Entry Signal                                     |
//| Check RSI crossover + EMA bounce + optional engulfing pattern   |
//+------------------------------------------------------------------+
bool IsEntrySignal(bool isBuy)
{
   double rsi[3];        // Current, previous, and 2 bars ago
   double ema[2];        // Current and previous EMA values
   double high[2], low[2], open[2], close[2];  // OHLC for engulfing check
   
   // Get RSI values (need 3 bars to confirm crossover)
   if(CopyBuffer(rsiM15Handle, 0, 0, 3, rsi) != 3)
   {
      return false;
   }
   
   // Get EMA values
   if(CopyBuffer(emaFastM15Handle, 0, 0, 2, ema) != 2)
   {
      return false;
   }
   
   // Get OHLC data for engulfing pattern (if enabled)
   if(UseEngulfingFilter)
   {
      if(CopyHigh(_Symbol, PERIOD_M15, 0, 2, high) != 2 ||
         CopyLow(_Symbol, PERIOD_M15, 0, 2, low) != 2 ||
         CopyOpen(_Symbol, PERIOD_M15, 0, 2, open) != 2 ||
         CopyClose(_Symbol, PERIOD_M15, 0, 2, close) != 2)
      {
         return false;
      }
   }
   ...
   {
      // BUY CONDITIONS:
      // 1. RSI crosses up from oversold (<30)
      bool rsiCrossUp = (rsi[2] < 30.0 && rsi[1] < 30.0 && rsi[0] > 30.0) || 
                        (rsi[1] < 30.0 && rsi[0] > rsi[1] && rsi[0] > 30.0);
      
      // 2. Price bounces off 50 EMA (price was below EMA, now above or close)
      bool emaBounce = (low[1] <= ema[1] && currentPrice > ema[0]) ||
                       (MathAbs(currentPrice - ema[0]) / SymbolInfoDouble(_Symbol, SYMBOL_POINT) <= 10);
      
      // 3. Optional: Bullish engulfing pattern
      bool engulfingOK = true;
      if(UseEngulfingFilter)
      {
         engulfingOK = IsBullishEngulfing(open, high, low, close);
      }
      
      return rsiCrossUp && emaBounce && engulfingOK;
   }
   else
   {
      // SELL CONDITIONS:
      // 1. RSI crosses down from overbought (>70)
      bool rsiCrossDown = (rsi[2] > 70.0 && rsi[1] > 70.0 && rsi[0] < 70.0) ||
                          (rsi[1] > 70.0 && rsi[0] < rsi[1] && rsi[0] < 70.0);
      
      // 2. Price bounces from 50 EMA (price was above EMA, now below or close)
      bool emaBounce = (high[1] >= ema[1] && currentPrice < ema[0]) ||
                       (MathAbs(currentPrice - ema[0]) / SymbolInfoDouble(_Symbol, SYMBOL_POINT) <= 10);
      
      // 3. Optional: Bearish engulfing pattern
      bool engulfingOK = true;
      if(UseEngulfingFilter)
      {
         engulfingOK = IsBearishEngulfing(open, high, low, close);
      }
      
      return rsiCrossDown && emaBounce && engulfingOK;
   }
}
...
//| Uses ATR(14) on M15: SL = 1.5x ATR, TP = 3.0x ATR             |
//+------------------------------------------------------------------+
void GetSLTP(double entryPrice, bool isBuy, double &sl, double &tp)
{
   ...
   double slDistance = atrValue * SL_ATR_Multiplier;  // 1.5 x ATR
   double tpDistance = atrValue * TP_ATR_Multiplier;  // 3.0 x ATR
   
   // Get symbol digits for proper price formatting
   ...
   // Validate SL and TP levels against broker's minimum distance
...
//| Calculate lot size to risk 1% of account based on SL distance   |
//+------------------------------------------------------------------+
double CalculateLotSize(double slPips)
{
   // Account information
   double accountBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   double riskAmount = accountBalance * (RiskPercent / 100.0);  // 1% of account
   ...
   // Handle different symbol types (Forex vs CFD/Indices)
   double pipValue;
   
   if(contractSize == 100000.0)  // Standard Forex pair
   {
      // For Forex: pip value = (tick size / point) * tick value
      pipValue = (tickSize / point) * tickValue;
   }
   else
   {
      // For CFDs/Indices: calculate pip value differently
   ...
   if(riskPerPip <= 0)
   {
      Print("Error: Invalid pip calculation. Using minimum lot size.");
      return SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   }
   
   // Calculate lot size
   double lotSize = riskAmount / riskPerPip;
   ...
   // Additional safety check for account margin
   double marginRequired = lotSize * SymbolInfoDouble(_Symbol, SYMBOL_MARGIN_INITIAL);
   double freeMargin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
   
   if(marginRequired > freeMargin * 0.9)  // Use only 90% of free margin
   {
      lotSize = (freeMargin * 0.9) / SymbolInfoDouble(_Symbol, SYMBOL_MARGIN_INITIAL);
      lotSize = MathFloor(lotSize / lotStep) * lotStep;
      ...
      Print("Warning: Lot size reduced due to insufficient margin. New lot size: ", lotSize);
   }
